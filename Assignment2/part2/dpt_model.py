...
class DeepPromptCLIP(nn.Module):
    ...
    def __init__(self, args, dataset, template="This is a photo of {}"):
        ...
        # tokenized_prompts = clip.tokenize(prompts).to(self.device) # TODO The prompts are already tokenized!!!!
        with torch.no_grad():
            text_features = clip_model.encode_text(prompts)
        text_features = text_features / text_features.norm(dim=-1, keepdim=True) # Calculates the L2 norm along the last dimension
        ...
        self.deep_prompt = nn.Parameter(torch.randn(size=(args.prompt_num, self.clip_model.visual.transformer.width)), requires_grad=True)
    ...
    def forward(self, image):
        ...
        image_features = image_features / image_features.norm(dim=-1, keepdim=True) # divide by the norm
        similarity = (image_features @ self.text_features.T) * self.logit_scale
        # similarity = (100 * image_features @ self.text_features.T).softmax(dim=-1)
        # similarity = self.clip_model.logit_scale * similarity
        # similarity = self.logit_scale * similarity
        return similarity
    ...
    def custom_encode_image(self, x):
        ...
        for i, tr in enumerate(image_encoder.transformer.resblocks):
            if i == self.injection_layer:
                x = torch.cat([self.deep_prompt.to(x.dtype).unsqueeze(1).repeat(1, x.size(1), 1), x])
            x = tr(x)
        #######################
        # END OF YOUR CODE    #
        #######################
